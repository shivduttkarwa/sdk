...
(function initStatsWater() {
  ...
  const IMAGES = ['assets/stats-1.png', 'assets/stats-2.png', 'assets/stats-3.png'];
  ...
  function mkShader(type, src) {
    ...
    if (!gl.getShaderParameter(s, gl.COMPILE_STATUS))
      console.error('[fluid]', gl.getShaderInfoLog(s));
    return s;
  }
  function mkProg(fs) {
  ...
  let vel0 = mkFBO(SIM_W, SIM_H, true),
    vel1 = mkFBO(SIM_W, SIM_H, true);
  let pre0 = mkFBO(SIM_W, SIM_H, true),
    pre1 = mkFBO(SIM_W, SIM_H, true);
  let dye0 = mkFBO(SIM_W, SIM_H, true),
    dye1 = mkFBO(SIM_W, SIM_H, true);
  ...
  function u2f(p, n, x, y) {
    gl.uniform2f(gl.getUniformLocation(p, n), x, y);
  }
  function u3f(p, n, x, y, z) {
  ...
  items.forEach((item, i) => {
    ...
    master.to(
      {},
      {
        ...
        ease: 'power2.out',
        onUpdate() {
          num.textContent = Math.round(this.progress() * target);
        },
        onComplete() {
  ...
  document.querySelectorAll('.sdk-stack__card').forEach((card) => {
    ...
    function step() {
      ...
      for (const p of pts) {
        ...
        const hw = p.tw * 0.5,
          hh = p.th * 0.5;
        const x0 = WALL_PAD + hw - p.hx,
          x1 = cw - WALL_PAD - hw - p.hx;
        const y0 = WALL_PAD + hh - p.hy,
          y1 = ch - WALL_PAD - hh - p.hy;

        if (x0 < x1) {
          if (p.x < x0) {
            p.x = x0;
            if (p.vx < -0.8) smashTag(p);
            p.vx = Math.abs(p.vx) * RESTIT;
          }
          if (p.x > x1) {
            p.x = x1;
            if (p.vx > 0.8) smashTag(p);
        ...
        if (y0 < y1) {
          if (p.y < y0) {
            p.y = y0;
            if (p.vy < -0.8) smashTag(p);
            p.vy = Math.abs(p.vy) * RESTIT;
          }
          if (p.y > y1) {
            p.y = y1;
            if (p.vy > 0.8) smashTag(p);
  ...
  (function initShowcaseWater() {
    ...
    function mkFBO(w, h, float) {
      ...
      gl.texImage2D(
        ...
        0,
        gl.RGBA,
        float ? gl.FLOAT : gl.UNSIGNED_BYTE,
        null,
      );
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
    ...
    const splatProg = mkProg(SPLAT_FS),
      advectProg = mkProg(ADVECT_FS),
      curlProg = mkProg(CURL_FS),
      vortProg = mkProg(VORTICITY_FS),
      divProg = mkProg(DIVERGENCE_FS),
      pressureProg = mkProg(PRESSURE_FS),
      gradProg = mkProg(GRADIENT_FS),
      renderProg = mkProg(RENDER_FS);
    ...
    let dye0 = mkFBO(SIM_W, SIM_H, true),
      dye1 = mkFBO(SIM_W, SIM_H, true);
    const divFBO = mkFBO(SIM_W, SIM_H, true),
      curlFBO = mkFBO(SIM_W, SIM_H, true);
    ...
    gl.texImage2D(
      ...
      gl.RGBA,
      gl.UNSIGNED_BYTE,
      new Uint8Array([5, 3, 10, 255]),
    );
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
    fetch('assets/hero-showcase-bg.png', { mode: 'cors' })
      .then((r) => r.blob())
      .then((b) => createImageBitmap(b, { imageOrientation: 'flipY', premultiplyAlpha: 'none' }))
      .then((bmp) => {
    ...
    (function render(t) {
      ...
      blit(dye1, advectProg, (p) => {
        ...
        u2f(p, 'u_texelSize', TXS[0], TXS[1]);
        u1f(p, 'u_dt', dt);
        u1f(p, 'u_dissipation', hasMouse ? DYE_DISS : 0.55);
      });
      [dye0, dye1] = [dye1, dye0];
      gl.bindFramebuffer(gl.FRAMEBUFFER, null);
      gl.viewport(0, 0, canvas.width || 1, canvas.height || 1);
