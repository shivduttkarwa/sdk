...
(() => {
  ...
  function initProcessTimeline() {
    ...
    if (!panels.length || !rail || !window.gsap || !window.ScrollTrigger) return;
...
(function initShowcaseWater() {
  ...
  function mkProg(fs){ const p=gl.createProgram(); gl.attachShader(p,mkShader(gl.VERTEX_SHADER,VS)); gl.attachShader(p,mkShader(gl.FRAGMENT_SHADER,fs)); gl.linkProgram(p); return p; }
  function mkFBO(w,h,float){
  ...
  section.addEventListener('mouseleave',()=>{ hasMouse=false; dmx=0; dmy=0; });
  ...
  (function render(t){
    ...
    if(hasMouse&&Math.abs(dmx)+Math.abs(dmy)>0.0001){
      const ar=(canvas.width||1)/(canvas.height||1);
      blit(vel1,splatProg,p=>{ tex(0,vel0.tex);u1i(p,'u_src',0);u2f(p,'u_point',mx,my);u2f(p,'u_aspect',ar,1.);u3f(p,'u_color',dmx*SPLAT_FORCE,dmy*SPLAT_FORCE,0.);u1f(p,'u_radius',SPLAT_RADIUS); });
      [vel0,vel1]=[vel1,vel0];
      blit(dye1,splatProg,p=>{ tex(0,dye0.tex);u1i(p,'u_src',0);u2f(p,'u_point',mx,my);u2f(p,'u_aspect',ar,1.);u3f(p,'u_color',1.,1.,1.);u1f(p,'u_radius',SPLAT_RADIUS); });
      [dye0,dye1]=[dye1,dye0]; }
    dmx=0; dmy=0;
    blit(curlFBO,curlProg,p=>{ tex(0,vel0.tex);u1i(p,'u_velocity',0);u2f(p,'u_texelSize',TXS[0],TXS[1]); });
    blit(vel1,vortProg,p=>{ tex(0,vel0.tex);u1i(p,'u_velocity',0);tex(1,curlFBO.tex);u1i(p,'u_curl',1);u2f(p,'u_texelSize',TXS[0],TXS[1]);u1f(p,'u_curl_strength',CURL_STR);u1f(p,'u_dt',dt); });
    [vel0,vel1]=[vel1,vel0];
    blit(divFBO,divProg,p=>{ tex(0,vel0.tex);u1i(p,'u_velocity',0);u2f(p,'u_texelSize',TXS[0],TXS[1]); });
    ...
    for(let i=0;i<PRESSURE_ITS;i++){ blit(pre1,pressureProg,p=>{ tex(0,pre0.tex);u1i(p,'u_pressure',0);tex(1,divFBO.tex);u1i(p,'u_divergence',1);u2f(p,'u_texelSize',TXS[0],TXS[1]); }); [pre0,pre1]=[pre1,pre0]; }
    blit(vel1,gradProg,p=>{ tex(0,pre0.tex);u1i(p,'u_pressure',0);tex(1,vel0.tex);u1i(p,'u_velocity',1);u2f(p,'u_texelSize',TXS[0],TXS[1]); }); [vel0,vel1]=[vel1,vel0];
    blit(vel1,advectProg,p=>{ tex(0,vel0.tex);u1i(p,'u_velocity',0);tex(1,vel0.tex);u1i(p,'u_quantity',1);u2f(p,'u_texelSize',TXS[0],TXS[1]);u1f(p,'u_dt',dt);u1f(p,'u_dissipation',VEL_DISS); }); [vel0,vel1]=[vel1,vel0];
    blit(dye1,advectProg,p=>{ tex(0,vel0.tex);u1i(p,'u_velocity',0);tex(1,dye0.tex);u1i(p,'u_quantity',1);u2f(p,'u_texelSize',TXS[0],TXS[1]);u1f(p,'u_dt',dt);u1f(p,'u_dissipation',hasMouse?DYE_DISS:0.55); }); [dye0,dye1]=[dye1,dye0];
