...
// ── Stats-v2: water reveal → smoke vanish cursor effect ──
(function initStatsV2() {
  const row = document.querySelector('.stats-v2-row');
  if (!row) return;
  ...
  // Smoke shader: diffuse outward each frame + fast decay = reveals as water, vanishes as smoke
  const SMOKE_FS = `precision highp float;
    ...
    void main(){
      vec2 px = 1./u_res;
      // Spread to neighbours with a looser, more equal kernel — smoke diffuses isotropically
      float c  = texture2D(u_trail, v_uv).r;
      float l  = texture2D(u_trail, v_uv - vec2(px.x, 0.)).r;
      float r  = texture2D(u_trail, v_uv + vec2(px.x, 0.)).r;
      float u  = texture2D(u_trail, v_uv + vec2(0., px.y)).r;
      float d  = texture2D(u_trail, v_uv - vec2(0., px.y)).r;
      float ul = texture2D(u_trail, v_uv + vec2(-px.x,  px.y)).r;
      float ur = texture2D(u_trail, v_uv + vec2( px.x,  px.y)).r;
      float dl = texture2D(u_trail, v_uv + vec2(-px.x, -px.y)).r;
      float dr = texture2D(u_trail, v_uv + vec2( px.x, -px.y)).r;
      // Center-dominant kernel (0.62) keeps the splash solid at its core;
      // small neighbour share creates a soft feathered edge without a hollow ring.
      // Weights sum to exactly 1.0 so density is conserved before decay.
      float diffused = c*0.62 + (l+r+u+d)*0.07 + (ul+ur+dl+dr)*0.025;
      // Moderate decay — visible for ~0.6 s then fades like smoke
      float trail = diffused * 0.964;

      // Water-like splat: wide Gaussian (k=50), fat splash, speed-modulated
      ...
      float splat = exp(-dist*dist*50.) * clamp(speed*6.+0.25, 0., 2.) * u_moving;
      gl_FragColor = vec4(clamp(trail + splat, 0., 1.), 0., 0., 1.);
    }`;

  // Render: smoke alpha has a very soft upper ramp — wispy, not crisp
  const RENDER_FS = `precision highp float;
    ...
    void main(){
      ...
      vec3 c2 = texture2D(u_bg2, vec2((v_uv.x-2.*seg)/seg,   v_uv.y)).rgb;
      vec3 bg = mix(mix(c0,c1,step(seg,v_uv.x)),c2,step(2.*seg,v_uv.x)) * 0.88;
      // Wide soft ramp — bright and full at centre, feathered edge, no darkening
      float a = smoothstep(0.0, 0.30, trail);
      gl_FragColor = vec4(bg, a);
    }`;

  function mkShader(type, src) {
    const s = gl.createShader(type);
    gl.shaderSource(s, src); gl.compileShader(s);
    if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) console.error('[stats-v2]', gl.getShaderInfoLog(s));
    return s;
  }
  function mkProg(fs) {
    const p = gl.createProgram();
  ...
  const smokeProg  = mkProg(SMOKE_FS);
  const renderProg = mkProg(RENDER_FS);

  const sL = {
    pos:    gl.getAttribLocation(smokeProg, 'a_pos'),
    trail:  gl.getUniformLocation(smokeProg, 'u_trail'),
    res:    gl.getUniformLocation(smokeProg, 'u_res'),
    mouse:  gl.getUniformLocation(smokeProg, 'u_mouse'),
    pmouse: gl.getUniformLocation(smokeProg, 'u_pmouse'),
    moving: gl.getUniformLocation(smokeProg, 'u_moving'),
    ar:     gl.getUniformLocation(smokeProg, 'u_ar'),
  };
  const rL = {
  ...
  gl.uniform1i(sL.trail, 0);
  gl.uniform2f(sL.res, SIM_W, SIM_H);
  ...
  let fboR = mkFBO(SIM_W, SIM_H), fboW = mkFBO(SIM_W, SIM_H);
  [fboR, fboW].forEach(f => {
    gl.bindFramebuffer(gl.FRAMEBUFFER, f.fbo);
    gl.viewport(0,0,SIM_W,SIM_H); gl.clearColor(0,0,0,1); gl.clear(gl.COLOR_BUFFER_BIT);
  });
  gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  ...
  row.addEventListener('mousemove', e => {
    ...
    mx = (e.clientX - r.left) / r.width;
    my = 1 - (e.clientY - r.top) / r.height;
    moving = 1; lastMove = performance.now();
  });
  ...
  function resize() {
    ...
    canvas.height = row.clientHeight;
    gl.useProgram(smokeProg);
    gl.uniform1f(sL.ar, (canvas.width || 1) / (canvas.height || 1));
  }
  resize();
  new ResizeObserver(resize).observe(row);
  ...
  (function render() {
    requestAnimationFrame(render);
    if (moving && performance.now() - lastMove > 100) moving = 0;
    ...
    gl.vertexAttribPointer(sL.pos, 2, gl.FLOAT, false, 0, 0);
    gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, fboR.tex);
    gl.uniform2f(sL.mouse,  smx, smy);
    gl.uniform2f(sL.pmouse, pmx, pmy);
    gl.uniform1f(sL.moving, moving);
    gl.drawArrays(gl.TRIANGLES, 0, 6);
    [fboR, fboW] = [fboW, fboR];
    ...
    gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, fboR.tex);
    gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, bgTex[0]);
    gl.activeTexture(gl.TEXTURE2); gl.bindTexture(gl.TEXTURE_2D, bgTex[1]);
    gl.activeTexture(gl.TEXTURE3); gl.bindTexture(gl.TEXTURE_2D, bgTex[2]);
    gl.clear(gl.COLOR_BUFFER_BIT);
    gl.drawArrays(gl.TRIANGLES, 0, 6);
  })();
})();
...
// ── Stats-v2 entrance animation ──
(function initStatsV2Anim() {
  if (!window.gsap || !window.ScrollTrigger) return;
  const section = document.querySelector('.stats-v2-section');
  if (!section) return;
